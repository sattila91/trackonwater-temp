...
mqttClient.on('connect', () => {
    ...
    mqttClient.subscribe(process.env.MQTT_TOPIC, (err) => {
        if (!err) {
            console.log('Subscribed to trackers/data');
        } else {
...
mqttClient.on('message', (topic, message) => {
    try {
        ...
        const trackerUid = data.tracker_uid;

        // Update the tracker data in memory
        trackersData[trackerUid] = data;

	totalMessages++;
	console.log(`Received MQTT message (${totalMessages} total)`);
        //console.log(`Updated tracker data for ${trackerUid}:`, data);
    } catch (error) {
...
// Start HTTPS server
if (sslEnabled) {
    // HTTPS szerver indítása, ha SSL engedélyezve van
    https.createServer(options, app).listen(port, () => {
        console.log(`Server running on https://localhost:${port}`);
    });
} else {
    // HTTP szerver indítása, ha SSL nincs engedélyezve
    app.listen(port, () => {
